Vagrant.configure('2') do |config|
  ...
  config.vm.define "cent7" do |cent7|
    ...
    cent7.vm.provision "shell", preserve_order: true, path: "scripts/cent7/install_puppet_agent.sh"

    # Build and install vim from source
    cent7.vm.provision "shell", preserve_order: true, path: "scripts/cent7/install_vim_src_deps.sh"
    cent7.vm.provision "shell", preserve_order: true, path: "scripts/cent7/install_vim_src_build.sh"

    # Build and install git from source
    cent7.vm.provision "shell", preserve_order: true,  path: "scripts/cent7/install_git_src_deps.sh"
    cent7.vm.provision "shell", preserve_order: true,  path: "scripts/install_git_src_build.sh"

    # Powerline goodness
    cent7.vm.provision "shell", preserve_order: true,  path: "scripts/cent7/install_powerline.sh"
